...
class TestDescriptorNumber:
    ...
    @pytest.mark.parametrize(
        'full_value',
        [sc.array(values=[1, 2], dims=['x']), sc.array(values=[[1], [2]], dims=['x', 'y']), object(), 1, 'string'],
    ...
    @pytest.mark.parametrize(
        'exponent, exception',
        [(DescriptorNumber('test', 2, unit='m'), UnitError), (DescriptorNumber('test', 2, variance=0.1), ValueError)],
