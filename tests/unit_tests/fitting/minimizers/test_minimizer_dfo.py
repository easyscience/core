...
class TestDFOFit:
    ...
    def test_generate_fit_function(self, minimizer: DFO) -> None:
        ...
        minimizer._object.get_fit_parameters = MagicMock(
            return_value=[mock_parm_1, mock_parm_2]
        )
    ...
    def test_make_model(self, minimizer: DFO) -> None:
        ...
        assert all(
            np.array([-0.01, -0.01]) == residuals_for_model(np.array([1111, 2222]))
        )
    ...
    def test_set_parameter_fit_result_no_stack_status(self, minimizer: DFO):
        ...
        minimizer._error_from_jacobian = MagicMock(
            return_value=np.array([[0.1, 0.0], [0.0, 0.2]])
        )
        ...
        minimizer._error_from_jacobian.assert_called_once_with(
            "jacobian", "resid", 0.95
        )
    ...
    def test_dfo_fit(self, minimizer: DFO, monkeypatch):
        ...
        monkeypatch.setattr(
            easyscience.fitting.minimizers.minimizer_dfo, "dfols", mock_dfols
        )
    ...
    def test_dfo_fit_no_scaling(self, minimizer: DFO, monkeypatch):
        ...
        assert all(mock_dfols.solve.call_args[0][1] == np.array([1.0, 2.0]))
        assert all(
            mock_dfols.solve.call_args[1]["bounds"][0] == np.array([-np.inf, 0.2])
        )
        assert all(mock_dfols.solve.call_args[1]["bounds"][1] == np.array([10.0, 20.0]))
