...
def test_collection_base_prevents_duplicates_by_unique_name():
    m1 = DummyModel('m1', make_parameter('p1', 1.0), unique_name='m1')
    m2 = DummyModel('m2', make_parameter('p2', 2.0), unique_name='m2')

    collection = CollectionBase('test', m1)
    collection._get_key = lambda _obj: 'same-key'
    collection.append(m2)

    assert len(collection) == 1
    assert collection[0] is m1


def test_collection_base_duplicate_identity_fallback_when_key_unavailable():
    m1 = DummyModel('m1', make_parameter('p1', 1.0), unique_name='m1')
    collection = CollectionBase('test', m1)

    def _broken_get_key(_obj):
        raise AttributeError('key unavailable')

    collection._get_key = _broken_get_key
    collection.append(m1)
